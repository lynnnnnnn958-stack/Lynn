...
║        Institutional-grade full version: Backtest + Live + Alpha Rigor + Execution Realism + Statistical Depth       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  v5 base retained:                                                               ║
║  · DataLayer / drawdown / backtest_metrics (Book Ch.7 exact formulas)               ║
║  · Regime five-dimension detection / detect_regime                                           ║
...
║                                                                              ║
║  v6 new additions:                                                                   ║
║  [FIX] Bull market full allocation: distribute by target gross exposure, BULL_STRONG → 90%+                      ║
║  [FIX] Volatility target: dynamically scale positions to target annualized volatility (10%)                      ║
║  [FIX] Drawdown control: auto-reduce position 50% if >5% drawdown, 75% if >10%                         ║
║  [A]   Alpha IC engine: IC/ICIR/t-stat validation; factors that fail are excluded                   ║
...
class DataLayer:
    ...
    def _synthetic(tickers: List[str], start: str, end: str):
        ...
        # Momentum persistence: prior period winners outperform next period (keeps CS-Momentum IC > 0)
...
def drawdown(return_series: pd.Series) -> pd.DataFrame:
    """Book Ch.7 Listing 7-14 to 7-16"""
...
def backtest_metrics(returns: pd.Series, rf: float = 0.03,
    ...
    if label:
        print(f"  [{label}] Annualized:{ann_ret:+.2%} Vol:{ann_vol:.2%} "
...
class AlphaICEngine:
    ...
    - IC decay: signal must not decay rapidly within holding period

    Factors that fail these standards are excluded from the portfolio
    Weight ∝ ICIR (more stable factors get higher weight)
    """
    ...
    def compute_ic_series(factor_matrix: pd.DataFrame,
        ...
        """
        Rolling calculation of IC time series for each factor
        ...
        for col in factor_matrix.columns:
            for t_idx in range(len(factor_matrix)):
                ...
                if isinstance(f_t, (int, float)) and np.isfinite(f_t):
                    # Single time point: cannot compute correlation, accumulate a period
                    pass
                # Use past 21 days of IC
                if t_idx >= 21:
    ...
    def build_validated_alpha(prices: pd.DataFrame,
        ...
        Each factor first computes IC series; only those passing threshold are used, weight=ICIR
        """
        ...
        # Compute IC for each factor (using IC series over past window)
...
class ExecutionCostModel:
    ...
    Stocks with high volatility and low liquidity have larger bid-ask spreads

    Market Impact (Almgren-Chriss square-root model):
    impact = σ_daily × √(trade_value / ADV)
    Large trade costs increase non-linearly; this is the biggest underestimation source in standard cost modeling
    ...
    def total_cost(self, vol_daily: float, adv_usd: float,
                   trade_usd: float) -> float:
        """Total cost for a single trade (as fraction of trade value)"""
    ...
    def adv_constrained_size(self, desired_usd: float,
        ...
        """
        ADV constraint: single day must not exceed max_pov of ADV
        Returns: (tradeable amount today, number of days needed to complete)
        """
...
# ══════════════════════════════════════════════════════════════════════════════
#  v9 Step5: SEC / Earnings / News / Insider Evidence Layer
# ══════════════════════════════════════════════════════════════════════════════

@dataclass
class FundamentalEventCard:
    """基本面 + 事件证据卡。只用于下单前审查，不是买卖命令。"""
    ticker: str
    sec_status: str
    latest_10q_or_10k: str = "N/A"
    recent_8k_count_90d: int = 0
    recent_form4_count_90d: int = 0
...
class SECEventClient:
    """
    SEC + Yahoo事件层。

    数据原则：
    - SEC部分使用官方data.sec.gov JSON端点（company_tickers / submissions / companyfacts）。
    - Yahoo部分只用作免费原型：earnings calendar 和 news headline，实盘前需要二次验证。
    - 若网络失败，不编造；返回 N/A / UNAVAILABLE，并把人工检查项写进卡片。
    """

    BASE_SEC = "https://data.sec.gov"
    TICKER_URL = "https://www.sec.gov/files/company_tickers.json"
    ETF_OR_INDEX = {
        'SPY','QQQ','GLD','TLT','SOXX','SMH','XLK','XLC','XLV','XLF','XLE','XLI','XLY','XLP','XLU','XLB','XLRE','CASH','TACTICAL_CASH'
    }
    ...
    def _headers() -> Dict[str, str]:
        ua = os.environ.get(
            "SEC_USER_AGENT",
            "CanyonResearch/0.1 educational-contact@example.com"
        )
        return {
            "User-Agent": ua,
            "Accept-Encoding": "gzip, deflate",
            "Host": "data.sec.gov" if "data.sec.gov" in SECEventClient.BASE_SEC else "www.sec.gov"
        }

    @staticmethod
    def _get_json(url: str, host: Optional[str] = None, timeout: int = 12) -> Optional[Dict]:
        try:
            import requests
            headers = SECEventClient._headers().copy()
            if host:
                headers["Host"] = host
    ...
    def load_ticker_cik_map() -> Dict[str, str]:
        if SECEventClient._ticker_cache is not None:
            return SECEventClient._ticker_cache
        try:
            import requests
            headers = SECEventClient._headers().copy()
            headers["Host"] = "www.sec.gov"
            r = requests.get(SECEventClient.TICKER_URL, headers=headers, timeout=15)
            r.raise_for_status()
            raw = r.json()
            mp = {}
            for _, item in raw.items():
                ...
                if ticker and cik:
                    mp[ticker] = cik
            SECEventClient._ticker_cache = mp
            return mp
        except Exception:
            SECEventClient._ticker_cache = {}
            return {}

    @staticmethod
    def cik_for_ticker(ticker: str) -> Optional[str]:
        ticker = ticker.upper().replace('-', '.').strip()
        if ticker in SECEventClient.ETF_OR_INDEX:
            return None
        return SECEventClient.load_ticker_cik_map().get(ticker)

    @staticmethod
    def sec_submissions(cik: str) -> Optional[Dict]:
        url = f"https://data.sec.gov/submissions/CIK{cik}.json"
        return SECEventClient._get_json(url, host="data.sec.gov")

    @staticmethod
    def sec_companyfacts(cik: str) -> Optional[Dict]:
        url = f"https://data.sec.gov/api/xbrl/companyfacts/CIK{cik}.json"
        return SECEventClient._get_json(url, host="data.sec.gov")

    @staticmethod
    def _recent_filing_stats(submissions: Dict) -> Dict:
        try:
            ...
            for form, ds in zip(forms, dates):
                try:
                    ...
                    continue
                age = (today - d).days
                if form in ('10-Q','10-K') and latest_report == "N/A":
                    latest_report = f"{form} {ds}"
                if age <= 90 and form == '8-K':
                    recent_8k += 1
                if age <= 90 and form in ('4','4/A'):
                    recent_f4 += 1
            return {
                'latest_10q_or_10k': latest_report,
                'recent_8k_count_90d': recent_8k,
                'recent_form4_count_90d': recent_f4,
            }
        except Exception:
            return {'latest_10q_or_10k': 'N/A', 'recent_8k_count_90d': 0, 'recent_form4_count_90d': 0}

    @staticmethod
    def _extract_yoy_from_companyfacts(facts: Dict, concept: str) -> Optional[float]:
        """从companyfacts里粗略提取最近FY或Q的同比变化。只做审查线索，不做估值定论。"""
        try:
            units = facts.get('facts', {}).get('us-gaap', {}).get(concept, {}).get('units', {})
            vals = []
            for unit_name, arr in units.items():
                if unit_name not in ('USD', 'USD/shares', 'shares'):
                    continue
                for x in arr:
                    val = x.get('val')
                    fy = x.get('fy')
                    ...
                    if form not in ('10-K','10-Q'):
                        continue
                    vals.append({'val': float(val), 'fy': int(fy), 'fp': str(fp), 'filed': filed or '', 'end': end or '', 'frame': frame or ''})
            if len(vals) < 2:
                return None
            # 优先同一季度/年度同比：取最新一条，再找fy-1且fp相同。
            vals = sorted(vals, key=lambda z: (z['end'], z['filed']))
            latest = vals[-1]
            prior_candidates = [v for v in vals if v['fy'] == latest['fy'] - 1 and v['fp'] == latest['fp']]
            if not prior_candidates:
                prior_candidates = vals[-5:-1]
            prior = prior_candidates[-1]
            if abs(prior['val']) < 1e-8:
                return None
            return float((latest['val'] - prior['val']) / abs(prior['val']))
        except Exception:
            return None

    @staticmethod
    def yfinance_events(ticker: str) -> Dict:
        out = {'earnings_date': 'N/A', 'news_headlines': []}
        try:
            import yfinance as yf
            tk = yf.Ticker(ticker)
            # Earnings date: yfinance不同版本返回结构不同，所以保守处理。
            try:
                cal = tk.calendar
                if isinstance(cal, pd.DataFrame) and not cal.empty:
                    ...
                    for v in cal.values():
                        if isinstance(v, (list, tuple)) and v:
                            ...
                            vals.append(str(v))
                    out['earnings_date'] = vals[0] if vals else 'N/A'
            except Exception:
                pass
            try:
                news = tk.news or []
                ...
                for n in news[:5]:
                    ...
                    if title:
                        heads.append(str(title)[:140])
                out['news_headlines'] = heads[:3]
            except Exception:
                pass
        except Exception:
            pass
...
class FundamentalEventLayer:
    """把价格证据卡升级成：价格 + SEC + 财报日期 + 新闻标题 + insider/Form 4 检查。"""

    @staticmethod
    def build_cards(plan: Dict[str, Dict[str, float]], max_names: int = 12) -> List[FundamentalEventCard]:
        tickers = []
        for sleeve, holdings in plan.items():
            for tk, w in holdings.items():
                tku = str(tk).upper()
                if w <= 0:
                    continue
                if tku in SECEventClient.ETF_OR_INDEX:
                    continue
                if tku not in tickers:
                    tickers.append(tku)
        ...
        for tk in tickers:
            evidence, risks, checks = [], [], []
            cik = SECEventClient.cik_for_ticker(tk)
            if not cik:
                cards.append(FundamentalEventCard(
                    ticker=tk,
                    sec_status="NO_CIK_OR_ETF",
                    evidence=["SEC CIK未匹配；如果是ETF/非美股/特殊ticker，需用其他数据源。"],
                    risks=["基本面事件证据不足。"],
                    manual_checks=["人工确认ticker类型、交易所、财报日期、新闻。"],
                    decision="REVIEW"
                ))
                continue
            ...
            sub = SECEventClient.sec_submissions(cik)
            if sub:
                st = SECEventClient._recent_filing_stats(sub)
                latest_report = st['latest_10q_or_10k']
                recent_8k = st['recent_8k_count_90d']
                recent_f4 = st['recent_form4_count_90d']
                evidence.append(f"SEC filings: latest={latest_report}; 8-K(90d)={recent_8k}; Form4(90d)={recent_f4}.")
            else:
                sec_status = "SEC_SUBMISSIONS_UNAVAILABLE"
                checks.append("SEC submissions下载失败或被限流；不要编造filing事件。")

            facts = SECEventClient.sec_companyfacts(cik)
            if facts:
                rev_yoy = SECEventClient._extract_yoy_from_companyfacts(facts, 'Revenues')
                if rev_yoy is None:
                    rev_yoy = SECEventClient._extract_yoy_from_companyfacts(facts, 'SalesRevenueNet')
                ni_yoy = SECEventClient._extract_yoy_from_companyfacts(facts, 'NetIncomeLoss')
                if rev_yoy is not None:
                    evidence.append(f"Revenue YoY proxy={rev_yoy:+.1%}（来自SEC companyfacts，粗略同比线索）。")
                else:
                    checks.append("SEC revenue同比未能稳定提取；需看10-Q/10-K原文。")
                if ni_yoy is not None:
                    evidence.append(f"Net income YoY proxy={ni_yoy:+.1%}（来自SEC companyfacts，粗略同比线索）。")
            else:
                if sec_status == "OK":
                    sec_status = "SEC_COMPANYFACTS_UNAVAILABLE"
                checks.append("SEC companyfacts下载失败或无标准us-gaap概念；不能给基本面同比结论。")

            yf_ev = SECEventClient.yfinance_events(tk)
            earnings_date = yf_ev.get('earnings_date', 'N/A')
            headlines = yf_ev.get('news_headlines', [])
            if earnings_date and earnings_date != 'N/A':
                checks.append(f"财报日期线索：{earnings_date}。短线下单前必须二次确认。")
            else:
                checks.append("财报日期未获取；短线下单前必须手动查财报日。")
            if headlines:
                evidence.append("Yahoo news headlines: " + " | ".join(headlines[:2]))
            else:
                checks.append("新闻标题未获取；下单前手动查当日新闻/盘前盘后异动。")

            if recent_8k >= 3:
                risks.append("90天内8-K较多：事件密集，短线波动和信息风险较高。")
            if recent_f4 >= 5:
                evidence.append("Form 4较多：近期管理层/内部人交易披露活跃，需进一步区分买入、卖出、期权归属。")
                checks.append("Form 4只能说明披露活跃；必须打开原始Form 4确认交易方向和性质。")
            if rev_yoy is not None and rev_yoy < -0.05:
                risks.append("收入同比为负：若做多，需要更强的事件/估值/反转理由。")
            if ni_yoy is not None and ni_yoy < -0.10:
                risks.append("净利润同比下滑明显：短线可以交易，但不应自动归为长期优质。")

            # Decision rule: conservative, not an automatic buy/sell.
            if sec_status == "OK" and len(risks) == 0 and len(evidence) >= 2:
                decision = "ALLOW_WITH_CHECKS"
            elif sec_status.startswith("SEC_") or len(risks) >= 2:
                decision = "REVIEW_OR_REDUCE"
            else:
                decision = "REVIEW"
    ...
    @staticmethod
    def print_cards(cards: List[FundamentalEventCard]):
        print("\n  v9 Step5 Fundamental/Event Evidence（SEC + 财报日期 + 新闻 + Form4线索）：")
        if not cards:
            print("  暂无单股需要SEC/Event证据；ETF只保留价格/板块证据。")
            return
        for c in cards:
            print(f"\n  [{c.ticker}] SEC={c.sec_status} decision={c.decision}")
            print(f"    latest report: {c.latest_10q_or_10k} | 8-K(90d)={c.recent_8k_count_90d} | Form4(90d)={c.recent_form4_count_90d}")
            if c.revenue_yoy is not None:
                print(f"    revenue_yoy_proxy: {c.revenue_yoy:+.1%}")
            if c.net_income_yoy is not None:
                print(f"    net_income_yoy_proxy: {c.net_income_yoy:+.1%}")
            print(f"    earnings_date_hint: {c.earnings_date}")
            for e in c.evidence[:3]:
                print(f"    证据: {e}")
            for r in c.risks[:3]:
                print(f"    风险: {r}")
            for chk in c.manual_checks[:3]:
                print(f"    检查: {chk}")
...
def main_v9_step5():
    print(f"\n{'═'*65}")
    print("  🏔  CANYON量化交易系统 v9 Step5")
    print("  Real Data × Evidence Guard × SEC/Event/Fundamental Layer")
    print(f"{'═'*65}")
    ...
    RealDataPolicy.print_policy(start, end, tickers)
    print("  · SEC层：data.sec.gov submissions/companyfacts；失败则标记N/A，不编造。")
    print("  · 事件层：yfinance earnings/news 仅作免费线索；下单前必须二次确认。")
    print("  · 可选：export SEC_USER_AGENT='CanyonResearch/0.1 your_email@example.com' 以减少SEC限流风险。")
    ...
    tactical = TacticalAlphaEngine.scan(prices, volumes, market, regime, max_names=10)
    print(f"\n  v9 Step5 TacticalAlphaEngine 候选：")
    if tactical.empty:
        print("  暂无通过阈值的短期非理性候选；TACTICAL资金保持现金。")
    else:
        for _, row in tactical.iterrows():
    ...
    raw_plan = CurrentPortfolioPlanner.build(regime, sleeve_weights, prices, market, tactical)
    print("\n  v9 Step5 原始模型组合（未过Exposure Guard）：")
    CurrentPortfolioPlanner.print_plan(raw_plan)

    guarded_plan, guard_notes = ExposureGuard.apply(raw_plan)
    ExposureGuard.print_report(raw_plan, guarded_plan, guard_notes)

    print("\n  v9 Step5 守门后组合（价格/暴露守门后，不等于自动下单）：")
    CurrentPortfolioPlanner.print_plan(guarded_plan)

    price_cards = EvidenceLayer.build_cards(guarded_plan, prices, volumes, market, tactical)
    EvidenceLayer.print_cards(price_cards, max_cards=14)

    fundamental_cards = FundamentalEventLayer.build_cards(guarded_plan, max_names=12)
    FundamentalEventLayer.print_cards(fundamental_cards)

    print("\n  v9 Step5 操作纪律：")
    print("  · 价格证据只回答：趋势/相对强度/量能/暴露是否合理。")
    print("  · SEC/Event证据回答：最近是否有财报、8-K、Form 4、收入/利润线索、新闻风险。")
    print("  · Form 4数量不是买入信号；必须打开原始申报确认是买入、卖出、期权归属还是计划交易。")
    print("  · 如果SEC或新闻下载失败，系统只能给REVIEW，不允许自动升级为ALLOW。")
    print("  · 下一步Step6做Trade Journal + Learning Engine：记录每次交易结果，让策略权重自己学习调整。")
...
# ══════════════════════════════════════════════════════════════════════════════
# v9 Step6: Trade Journal + Learning Engine
# ══════════════════════════════════════════════════════════════════════════════

class Step6TradeJournal:
    """
    交易日志层：只记录候选/人工确认后的交易，不伪造历史收益。

    状态定义：
    - WATCHLIST: 系统候选，等待人工检查新闻/财报/流动性。
    - PLANNED: 人工决定准备交易，但还未成交。
    - OPEN: 已成交持仓。
    - CLOSED: 已平仓，可用于学习。

    LearningEngine 只读取 CLOSED 交易；WATCHLIST/PLANNED/OPEN 不参与收益学习。
    """
    REQUIRED_COLUMNS = [
        'trade_id','created_at','status','ticker','sleeve','thesis_type','side',
        'planned_weight','entry_date','entry_price','exit_date','exit_price',
        'realized_return','holding_days','regime','decision','evidence_score',
        'risk_notes','checklist','source'
    ]

    def __init__(self, path: str = 'canyon_trade_journal.csv'):
        self.path = Path(path)
        self._ensure_file()

    def _ensure_file(self):
        if not self.path.exists():
            pd.DataFrame(columns=self.REQUIRED_COLUMNS).to_csv(self.path, index=False)
        else:
            ...
            for col in self.REQUIRED_COLUMNS:
                if col not in df.columns:
                    ...
                    changed = True
            if changed:
                df[self.REQUIRED_COLUMNS].to_csv(self.path, index=False)
    ...
    def append_watchlist_from_plan(self, plan: Dict[str, Dict[str, float]], price_cards: List, fundamental_cards: Dict, regime_label: str):
        """把本次系统输出写入 WATCHLIST。去重：同一天同ticker同sleeve不重复写。"""
        df = self.load()
        ...
        try:
            for c in price_cards:
                pcards[getattr(c, 'ticker', '')] = c
        except Exception:
            pcards = {}

        for sleeve, holdings in plan.items():
            for ticker, weight in holdings.items():
                if ticker.endswith('CASH') or ticker == 'CASH' or abs(float(weight)) < 1e-6:
                    continue

                exists = False
                if len(df) > 0:
                    exists = ((df['created_at'].astype(str).str[:10] == today) &
                              (df['ticker'].astype(str) == ticker) &
                              (df['sleeve'].astype(str) == sleeve)).any()
                if exists:
                    continue

                pc = pcards.get(ticker)
                fcard = fundamental_cards.get(ticker) if isinstance(fundamental_cards, dict) else None
                decision = getattr(pc, 'decision', 'REVIEW') if pc is not None else 'REVIEW'
                evidence_score = getattr(pc, 'evidence_score', np.nan) if pc is not None else np.nan
                risk_notes = []
                checklist = []
                if pc is not None:
                    risk_notes += list(getattr(pc, 'risks', []) or [])
                    checklist += list(getattr(pc, 'checklist', []) or [])
                if fcard is not None:
                    risk_notes += list(getattr(fcard, 'risks', []) or [])
                    checklist += list(getattr(fcard, 'checks', []) or [])
                    f_decision = getattr(fcard, 'decision', '')
                    if f_decision and f_decision not in ('ALLOW', 'OK'):
                        decision = f_decision

                if 'TACTICAL' in sleeve:
                    thesis = 'short_term_irrationality_or_continuation'
                    side = 'LONG' if weight > 0 else 'SHORT'
                elif 'SECTOR' in sleeve:
                ...
                rows.append({
                    ...
                    'thesis_type': thesis,
                    'side': side,
                    'planned_weight': round(float(weight), 6),
                    'entry_date': '',
                    'entry_price': '',
                    'exit_date': '',
                    'exit_price': '',
                    ...
                    'risk_notes': ' | '.join(map(str, risk_notes[:8])),
                    'checklist': ' | '.join(map(str, checklist[:8])),
                    'source': 'v9_step6_system_watchlist_real_data'
                })
    ...
    def print_summary(self):
        df = self.load()
        print("\n  v9 Step6 Trade Journal：")
        print(f"  路径: {self.path.resolve()}")
        print(f"  总记录: {len(df)}")
        if len(df) == 0:
            print("  暂无记录。")
            return
        print("  状态统计:")
        for status, n in df['status'].fillna('UNKNOWN').value_counts().items():
            print(f"    {status:<10} {int(n)}")
        recent = df.tail(10)
        print("  最近10条:")
        for _, r in recent.iterrows():
            print(f"    {str(r['created_at'])[:10]} {str(r['status']):<9} {str(r['sleeve']):<16} {str(r['ticker']):<6} w={r['planned_weight']} decision={r['decision']}")


class Step6LearningEngine:
    """
    自学习层：只从真实 CLOSED 交易学习，不从候选/模拟结果学习。
    输出为建议调权，不直接改仓位、不自动下单。
    """
    MIN_TRADES_PER_BUCKET = 5

    @staticmethod
    def _closed_trades(journal_df: pd.DataFrame) -> pd.DataFrame:
        if journal_df is None or len(journal_df) == 0:
            return pd.DataFrame()
        df = journal_df.copy()
        df = df[df['status'].astype(str).str.upper() == 'CLOSED']
        if len(df) == 0:
            return df
    ...
    def bucket_stats(df: pd.DataFrame, bucket_col: str) -> pd.DataFrame:
        closed = Step6LearningEngine._closed_trades(df)
        if closed.empty or bucket_col not in closed.columns:
            return pd.DataFrame()
        rows = []
        for key, g in closed.groupby(bucket_col):
            rets = g['realized_return'].astype(float)
            wins = rets[rets > 0]
            losses = rets[rets <= 0]
            ...
            profit_factor = abs(wins.sum() / losses.sum()) if len(losses) and losses.sum() != 0 else np.inf
            rows.append({
                bucket_col: key,
                'n': len(g),
                ...
                'suggestion': Step6LearningEngine._suggestion(len(g), expectancy, win_rate, profit_factor)
            })
        return pd.DataFrame(rows).sort_values(['n','avg_return'], ascending=[False, False])

    @staticmethod
    def _suggestion(n: int, avg_return: float, win_rate: float, profit_factor: float) -> str:
        if n < Step6LearningEngine.MIN_TRADES_PER_BUCKET:
            return 'NO_CHANGE_INSUFFICIENT_SAMPLE'
        if avg_return > 0.01 and win_rate >= 0.55 and profit_factor > 1.3:
            return 'CONSIDER_UPWEIGHT_WITH_CAP'
        if avg_return < -0.005 or (win_rate < 0.45 and profit_factor < 1.0):
            return 'DOWNWEIGHT_OR_COOLDOWN'
        return 'NO_CHANGE_MONITOR'

    @staticmethod
    def print_report(journal_df: pd.DataFrame):
        closed = Step6LearningEngine._closed_trades(journal_df)
        print("\n  v9 Step6 Learning Engine：")
        print(f"  可学习CLOSED交易数: {len(closed)}")
        if len(closed) < 5:
            print("  结论: 样本不足，不自动调权。现在只建立日志和候选审查流程。")
            print("  规则: 至少有5笔CLOSED交易后，才开始对sleeve/thesis给出调权建议。")
            return
        for col, title in [('sleeve','按账户'), ('thesis_type','按交易逻辑'), ('regime','按市场状态')]:
            stats = Step6LearningEngine.bucket_stats(journal_df, col)
            print(f"\n  {title}表现:")
            if stats.empty:
                print("    暂无可统计数据。")
                continue
            for _, r in stats.iterrows():
                print(f"    {r[col]:<30} n={int(r['n']):<3} avg={r['avg_return']:+.2%} win={r['win_rate']:.0%} pf={r['profit_factor']:.2f} -> {r['suggestion']}")

    @staticmethod
    def generate_manual_update_template(path: str = 'trade_update_template.csv'):
        cols = ['trade_id','status','entry_date','entry_price','exit_date','exit_price','realized_return','holding_days','notes']
        p = Path(path)
        if not p.exists():
...
def main_v9_step6():
    print(f"\n{'═'*65}")
    print("  🏔  CANYON量化交易系统 v9 Step6")
    print("  Real Data × SEC/Event Evidence × Trade Journal × Learning Engine")
    print(f"{'═'*65}")

    # 先运行Step5，得到真实数据、守门后组合和证据卡。
    result = main_v9_step5()

    # 把守门后组合写入交易日志。注意：这不是下单，只是WATCHLIST。
    guarded_plan = result.get('guarded_plan', {})
    price_cards = result.get('price_cards', [])
    fundamental_cards_list = result.get('fundamental_cards', [])
    fcard_map = {}
    try:
        for c in fundamental_cards_list:
            fcard_map[getattr(c, 'ticker', '')] = c
    except Exception:
        fcard_map = {}

    journal = Step6TradeJournal('canyon_trade_journal.csv')
    added = journal.append_watchlist_from_plan(
        guarded_plan,
        price_cards,
        fcard_map,
        result.get('regime', 'UNKNOWN')
    )
    print(f"\n  v9 Step6 本次新增WATCHLIST记录: {len(added)}")
    journal.print_summary()

    df = journal.load()
    Step6LearningEngine.print_report(df)
    template = Step6LearningEngine.generate_manual_update_template('trade_update_template.csv')
    print("\n  v9 Step6 操作纪律：")
    print("  · WATCHLIST不是订单；只有人工确认新闻/财报/流动性后，才可以改成PLANNED或OPEN。")
    print("  · Learning Engine只从CLOSED真实交易学习，不从模型候选或模拟收益学习。")
    print("  · 如果某个thesis连续亏损，后续会进入DOWNWEIGHT_OR_COOLDOWN。")
    print(f"  · 手动更新模板已准备: {template.resolve()}")
    print("  · 下一步Step7做Broker/Execution Gate：把'候选'和'真实下单'完全隔离，加入预交易检查。")
    return result



# ══════════════════════════════════════════════════════════════════════════════
#  v9 STEP 7 — Broker / Execution Gate
#  目标：把“候选”与“真实下单”完全隔离。
#  这个模块不会自动下单，只生成预交易审核表和订单草稿表。
# ══════════════════════════════════════════════════════════════════════════════
...
class Step7ExecutionGate:
    """
    Broker/Execution Gate：下单前最后一道闸门。

    设计原则：
    1. WATCHLIST 永远不是订单。
    2. REVIEW/REVIEW_OR_REDUCE 默认不能直接下单。
    3. TACTICAL 单票默认上限更低，避免短线信号重仓。
    4. 所有真实订单必须先经过人工检查：新闻、财报日期、流动性、价差、thesis。
    5. 本模块只输出 CSV 审核表，不连接 broker，不自动交易。
    """

    ACTIVE_STATUSES = {'WATCHLIST', 'PLANNED'}
    REDUCE_DECISIONS = {'REVIEW_OR_REDUCE', 'REDUCE_OR_SKIP'}
    BLOCK_DECISIONS = {'BLOCK', 'SKIP', 'REJECT'}

    MAX_WEIGHT_BY_SLEEVE = {
        'TACTICAL': 0.025,
    ...
    REVIEW_COLUMNS = [
        'trade_id','created_at','ticker','sleeve','side','planned_weight','approved_weight',
        'decision','gate_status','reason','manual_news_check','earnings_date_check',
        'liquidity_check','spread_check','thesis_confirmed','risk_notes','checklist',
        'order_intent','order_type','limit_price','time_in_force','notes'
    ]

    ORDER_TICKET_COLUMNS = [
        'trade_id','ticker','side','approved_weight','order_intent','order_type','limit_price',
        'time_in_force','notional_usd','shares_estimate','broker','status','notes'
    ]
    ...
    @classmethod
    def cap_weight(cls, sleeve: str, planned_weight: float, decision: str) -> Tuple[float, str]:
        key = cls._sleeve_key(sleeve)
        cap = cls.MAX_WEIGHT_BY_SLEEVE.get(key, 0.025)
        w = abs(float(planned_weight)) if pd.notna(planned_weight) else 0.0
        reason = []
        if w > cap:
            reason.append(f"planned_weight {w:.1%} > {key} cap {cap:.1%}; capped")
            w = cap
        d = str(decision).upper()
        if d in cls.REDUCE_DECISIONS:
            reason.append("decision requires reduction; half-size max before manual approval")
            w = min(w * 0.5, cap * 0.5)
        if d in cls.BLOCK_DECISIONS:
            reason.append("decision blocks order")
            w = 0.0
        return float(round(w, 6)), ' | '.join(reason)

    @classmethod
    def evaluate_row(cls, r: pd.Series) -> ExecutionGateDecision:
        decision = str(r.get('decision', 'REVIEW')).upper()
        sleeve = str(r.get('sleeve', 'TACTICAL'))
        planned = pd.to_numeric(pd.Series([r.get('planned_weight', 0)]), errors='coerce').fillna(0).iloc[0]
        approved, cap_reason = cls.cap_weight(sleeve, planned, decision)

        required_checks = 'news; earnings_date; liquidity; bid_ask_spread; thesis_confirmation'
        reasons = []
        if cap_reason:
            reasons.append(cap_reason)
        if decision != 'ALLOW':
            reasons.append(f"decision={decision}; manual review required")
        if approved <= 0:
            gate_status = 'BLOCKED'
        else:
            gate_status = 'PENDING_MANUAL_CHECKS'
        ...
        return ExecutionGateDecision(
            ...
            reason=' | '.join(reasons) if reasons else 'size within cap; still needs manual checks',
            required_checks=required_checks
        )

    @classmethod
    def build_review_file(cls,
                          journal_path: str = 'canyon_trade_journal.csv',
                          output_path: str = 'execution_gate_review.csv') -> pd.DataFrame:
        jp = Path(journal_path)
        if not jp.exists():
            pd.DataFrame(columns=cls.REVIEW_COLUMNS).to_csv(output_path, index=False)
            return pd.DataFrame(columns=cls.REVIEW_COLUMNS)

        journal = pd.read_csv(jp)
        if journal.empty:
            pd.DataFrame(columns=cls.REVIEW_COLUMNS).to_csv(output_path, index=False)
            return pd.DataFrame(columns=cls.REVIEW_COLUMNS)

        df = journal[journal['status'].astype(str).str.upper().isin(cls.ACTIVE_STATUSES)].copy()
        if df.empty:
            pd.DataFrame(columns=cls.REVIEW_COLUMNS).to_csv(output_path, index=False)
            return pd.DataFrame(columns=cls.REVIEW_COLUMNS)

        # 只保留最新一批/近期候选，避免旧 watchlist 一直污染审核表。
        if 'created_at' in df.columns:
            df['_created_date'] = df['created_at'].astype(str).str[:10]
            latest_date = df['_created_date'].max()
            df = df[df['_created_date'] == latest_date].copy()

        rows = []
        for _, r in df.iterrows():
            gd = cls.evaluate_row(r)
            rows.append({
                'trade_id': gd.trade_id,
                'created_at': r.get('created_at',''),
                'ticker': gd.ticker,
                'sleeve': gd.sleeve,
                'side': gd.side,
                'planned_weight': round(gd.planned_weight, 6),
                'approved_weight': round(gd.approved_weight, 6),
                'decision': gd.decision,
                'gate_status': gd.gate_status,
                'reason': gd.reason,
        ...
        for col in cls.REVIEW_COLUMNS:
            if col not in out.columns:
                out[col] = ''
        out[cls.REVIEW_COLUMNS].to_csv(output_path, index=False)
        return out[cls.REVIEW_COLUMNS]

    @classmethod
    def build_order_ticket_from_review(cls,
                                       review_path: str = 'execution_gate_review.csv',
                                       output_path: str = 'pre_trade_order_ticket.csv',
                                       capital_usd: float = 10000.0) -> pd.DataFrame:
        """
        只有当人工把所有检查项填成 YES，且 order_intent=PAPER 或 LIVE，才生成订单草稿。
        默认不会生成真实订单。
        """
        rp = Path(review_path)
        if not rp.exists():
            pd.DataFrame(columns=cls.ORDER_TICKET_COLUMNS).to_csv(output_path, index=False)
            return pd.DataFrame(columns=cls.ORDER_TICKET_COLUMNS)
        ...
        yes_cols = ['manual_news_check','earnings_date_check','liquidity_check','spread_check','thesis_confirmed']
        mask = pd.Series(True, index=rv.index)
        for c in yes_cols:
            mask &= rv[c].astype(str).str.upper().eq('YES')
        mask &= rv['gate_status'].astype(str).str.upper().ne('BLOCKED')
        mask &= rv['order_intent'].astype(str).str.upper().isin(['PAPER','LIVE'])
        approved = rv[mask].copy()

        rows = []
        for _, r in approved.iterrows():
            w = float(pd.to_numeric(pd.Series([r.get('approved_weight',0)]), errors='coerce').fillna(0).iloc[0])
            notional = abs(w) * capital_usd
            rows.append({
                'trade_id': r.get('trade_id',''),
                'ticker': r.get('ticker',''),
                'side': r.get('side','LONG'),
                'approved_weight': w,
                'order_intent': r.get('order_intent','PAPER'),
                'order_type': r.get('order_type','LIMIT_ONLY'),
                'limit_price': r.get('limit_price',''),
                'time_in_force': r.get('time_in_force','DAY'),
                'notional_usd': round(notional, 2),
                'shares_estimate': '',
                'broker': 'MANUAL_OR_PAPER_ONLY',
                'status': 'DRAFT_NOT_SENT',
                'notes': r.get('notes','')
            })
        out = pd.DataFrame(rows)
        for col in cls.ORDER_TICKET_COLUMNS:
            if col not in out.columns:
                out[col] = ''
        out[cls.ORDER_TICKET_COLUMNS].to_csv(output_path, index=False)
        return out[cls.ORDER_TICKET_COLUMNS]

    @staticmethod
    def print_report(review_df: pd.DataFrame, order_df: pd.DataFrame):
        print("\n  v9 Step7 Broker / Execution Gate：")
        print("  目标: WATCHLIST 与真实下单彻底隔离。")
        print("  结果: 本模块只生成审核表和订单草稿，不自动连接 broker。")
        print(f"  审核候选数: {len(review_df)}")
        if len(review_df) > 0:
            print("  Gate状态:")
            for st, n in review_df['gate_status'].value_counts().items():
                print(f"    {st:<26} {int(n)}")
            print("  最近候选:")
            for _, r in review_df.head(12).iterrows():
                print(f"    {r['ticker']:<6} {r['sleeve']:<16} planned={float(r['planned_weight']):.1%} approved_max={float(r['approved_weight']):.1%} status={r['gate_status']} decision={r['decision']}")
        print(f"  已生成订单草稿数: {len(order_df)}")
        if len(order_df) == 0:
            print("  当前没有订单草稿。正常：你还没有把人工检查项改成 YES，也没有设置 order_intent=PAPER/LIVE。")
        print("\n  v9 Step7 操作纪律：")
        print("  · execution_gate_review.csv 是人工审核表，不是下单文件。")
        print("  · 所有检查项必须是 YES，order_intent 必须是 PAPER 或 LIVE，才会进入 pre_trade_order_ticket.csv。")
        print("  · TACTICAL 默认单票最多 2.5%；REVIEW_OR_REDUCE 默认再减半。")
        print("  · LIMIT_ONLY 是默认订单类型；不要用 market order 追高。")
        print("  · Step8 再做 Dashboard：把 regime、sleeve、evidence、journal、execution gate 汇总成一个每日PM报告。")


def main_v9_step7():
    print(f"\n{'═'*65}")
    print("  🏔  CANYON量化交易系统 v9 Step7")
    print("  Trade Journal × Broker/Execution Gate × Pre-trade Checklist")
    print(f"{'═'*65}")

    # 先运行 Step6，刷新真实数据候选和交易日志。
    result = main_v9_step6()

    review = Step7ExecutionGate.build_review_file(
        journal_path='canyon_trade_journal.csv',
        output_path='execution_gate_review.csv'
    )
    orders = Step7ExecutionGate.build_order_ticket_from_review(
        review_path='execution_gate_review.csv',
        output_path='pre_trade_order_ticket.csv',
        capital_usd=10000.0
    )
    Step7ExecutionGate.print_report(review, orders)
    print(f"\n  文件已生成:")
    print(f"  · {Path('execution_gate_review.csv').resolve()}")
    print(f"  · {Path('pre_trade_order_ticket.csv').resolve()}")
    return {'step6_result': result, 'review': review, 'orders': orders}


if __name__ == '__main__':
    result = main_v9_step7()
