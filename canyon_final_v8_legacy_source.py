...
║  [FIX] Drawdown control: auto reduce position 50% at >5% drawdown, 75% at >10%                         ║
...
class DataLayer:
    """Yahoo Finance preferred; auto-fallback to built-in synthetic data if network unavailable"""
    ...
    def _synthetic(tickers: List[str], start: str, end: str):
        ...
        # Momentum persistence: last period winners stronger next period (makes CS-Momentum IC > 0)
...
def backtest_metrics(returns: pd.Series, rf: float = 0.03,
    ...
    if label:
        print(f"  [{label}] Ann.Ret:{ann_ret:+.2%} Volatility:{ann_vol:.2%} "
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. [NEW] Alpha IC Engine (addressing "Alpha Rigor 4/10")
# ══════════════════════════════════════════════════════════════════════════════
...
class AlphaICEngine:
    ...
    Weight ∝ ICIR (more stable = higher weight)
    """
        ...
        for col in factor_matrix.columns:
            for t_idx in range(len(factor_matrix)):
                ...
                if isinstance(f_t, (int, float)) and np.isfinite(f_t):
                    # Single time point: cannot compute correlation, accumulate a segment
                    pass
    ...
    def build_validated_alpha(prices: pd.DataFrame,
        ...
        Factor library:
        F1 = 12-1 month cross-sectional momentum (most important)
        F2 = Short-term reversal (5-day)
        F3 = Volume-price directionality
        F4 = Relative strength (vs market)
        F5 = Low volatility position (low-volatility factor)

        Each factor computes IC series first; only used if it passes threshold; weight=ICIR
        """
        ...
        # Calculate IC for each factor over past window periods
        window = min(252, n - 22)
        factors_raw = {}
        ...
        # Compute IC for each factor (using IC series from past window)
...
# ══════════════════════════════════════════════════════════════════════════════
# 3. [NEW] Execution Cost Model (addressing "Execution Realism 3/10")
# ══════════════════════════════════════════════════════════════════════════════
...
class ExecutionCostModel:
    ...
    High-volatility, low-liquidity stocks have larger bid-ask spreads

    Market impact (Almgren-Chriss square-root model):
    impact = σ_daily × √(trade_value / ADV)
    Large-order trading costs increase nonlinearly; biggest underestimation source in standard cost modeling

    ADV constraint:
    Single trade must not exceed max_pov = 5% of average daily volume
    Excess beyond limit must be executed over multiple days
    """

    def __init__(self,
                 spread_k:  float = 0.10,   # bid-ask spread coefficient
                 impact_k:  float = 0.10,   # market impact coefficient
                 max_pov:   float = 0.05,   # max participation rate 5%
    ...
    def spread_cost(self, vol_daily: float, adv_usd: float) -> float:
        """Bid-ask spread cost (as proportion of trade value)"""
        if adv_usd <= 0:
            return self.max_bps
        # spread ∝ σ / √ADV
    ...
    def total_cost(self, vol_daily: float, adv_usd: float,
                   trade_usd: float) -> float:
        """Total cost of a single trade (as proportion of trade value)"""
    ...
    def adv_constrained_size(self, desired_usd: float,
        ...
        Returns: (today tradeable amount, days needed to complete)
        """
    ...
    def compute_portfolio_tc(self, weights_new: pd.Series,
        ...
        """
        Calculate total trading cost of a full portfolio rebalance
        """
        total_tc  = 0.0
        tc_detail = {}
...
# ══════════════════════════════════════════════════════════════════════════════
# 4. [NEW] Statistical Depth (addressing "Statistical Depth 4/10")
# ══════════════════════════════════════════════════════════════════════════════
...
class StatisticalDepth:
    ...
    - Newey-West corrected Sharpe: daily returns have autocorrelation; plain Sharpe is inflated
    - Factor exposure decomposition: prove strategy has pure Alpha, not high Beta
    ...
    def newey_west_sharpe(returns: pd.Series, rf: float = 0.03,
        ...
        Daily returns have autocorrelation → plain vol understated → plain Sharpe inflated
    ...
    def print_full_report(returns: pd.Series, market: pd.Series,
        ...
        print(f"  Factor Exposure: Alpha={expo['alpha_ann']:+.2%} "
              ...
              f"{'✅Pure Alpha' if expo['pure_alpha'] else '⚠️Beta-biased'}")
        print(f"  Drawdown Analysis: MaxDD={dd_d['max_dd']:.2%} "
              ...
              f"  Drawdown Analysis: MaxDD={dd_d['max_dd']:.2%} 持续{dd_d['max_duration_days']}天 未恢复")
...
class Regime(Enum):
    ...
    BULL_NORMAL = ( 1, "Normal Bull", "Offense First",  0.12, -0.00)
    ...
    def target_gross_exposure(self) -> float:
        """Target gross long exposure (v6 new: ensures full allocation in bull market)"""
...
def detect_regime(market: pd.Series,
    ...
    """Five-dimensional market regime detection"""
    if len(market) < 60:
        return Regime.NEUTRAL, {}
    m = market.dropna()

    # Dimension 1: SMA trend
    ...
    # Dimension 5: Short-term momentum
...
def trend_signals(prices: pd.DataFrame,
    ...
    """Book Ch.5: Golden cross = long, death cross = short"""
    results = {}
...
def cross_sectional_momentum(prices: pd.DataFrame,
    ...
    """Book Ch.6: Long top 25%, short bottom 25%"""
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
# 11. [NEW] Volatility target + drawdown control (improve Sharpe, reduce drawdown)
# ══════════════════════════════════════════════════════════════════════════════

class VolTargeter:
    """
    Volatility target: dynamically scale position so portfolio annualized volatility approaches target
    Target volatility = 10%
    Current vol high → reduce position; current vol low → increase position

    Core mechanism for improving Sharpe:
    - Low-volatility period: signal quality high, amplify position
    - High-volatility period: signal quality poor, reduce position
    ...
    def scale(self, weights: pd.Series, returns: pd.DataFrame) -> Tuple[pd.Series, float]:
        """
        Returns scaled weights + scaling factor
        """
...
class DrawdownController:
    """
    Drawdown control: dynamically reduce position to prevent consecutive losses
    Drawdown > 5%  → position × 0.5
    Drawdown > 10% → position × 0.25

    Core mechanism for reducing max drawdown:
    - Does not rely on fixed stop-loss; dynamically adjusts based on current drawdown
    ...
    def scale_weights(self, weights: pd.Series) -> pd.Series:
        ...
        if sc < 1.0:
            pass  # do not print; too verbose in backtest
        return weights * sc
