...
║                                                                              ║
║  v6 additions:                                                                   ║
║  [FIX] Bull market full allocation: assign by target gross exposure, BULL_STRONG → 90%+                      ║
║  [FIX] Volatility target: dynamically scale position to target annualized volatility (10%)                      ║
║  [FIX] Drawdown control: reduce position 50% when drawdown >5%, 75% when >10%                         ║
║  [A]   Alpha IC engine: IC/ICIR/t-stat validation; factors failing threshold are excluded                   ║
...
class DataLayer:
    ...
    def _synthetic(tickers: List[str], start: str, end: str):
        """
        Synthetic data: precise simulation of 2020-2024 market cycles
        ...
        # Momentum persistence: prior-period winners outperform next period (makes CS-Momentum IC > 0)
...
class AlphaICEngine:
        ...
        for col in factor_matrix.columns:
            for t_idx in range(len(factor_matrix)):
                ...
                if isinstance(f_t, (int, float)) and np.isfinite(f_t):
                    # Single time point: cannot compute correlation, accumulate more
                    pass
                # Use IC from past 21 days
                if t_idx >= 21:
    ...
    def icir_weights(factor_validations: Dict[str, Dict]) -> Dict[str, float]:
        """
        ICIR weighting: combine validated factors by ICIR weight
        """
    ...
    def build_validated_alpha(prices: pd.DataFrame,
        ...
        Each factor's IC series is computed first; only factors passing the threshold are used, weight=ICIR
        """
...
class ExecutionCostModel:
    ...
    Higher volatility or lower liquidity stocks have larger bid-ask spreads

    Market Impact (Almgren-Chriss square-root model):
    impact = σ_daily × √(trade_value / ADV)
    Large-order transaction costs increase nonlinearly; most commonly underestimated cost
    ...
    def total_cost(self, vol_daily: float, adv_usd: float,
                   trade_usd: float) -> float:
        """Total transaction cost for a single trade (as fraction of trade value)"""
    ...
    def adv_constrained_size(self, desired_usd: float,
        ...
        """
        ADV constraint: single-day trade must not exceed max_pov of ADV
        Returns: (today's tradeable amount, number of days needed to complete)
        """
    ...
    def compute_portfolio_tc(self, weights_new: pd.Series,
        ...
        """
        Compute total transaction cost for a full portfolio rebalance
        """
        total_tc  = 0.0
        tc_detail = {}
...
class CurrentPortfolioPlanner:
    ...
    def _sector_rotation_weights(prices: pd.DataFrame, market: pd.Series, sleeve_weight: float) -> Dict[str, float]:
        scores = {}
        m20 = float(market.pct_change(20).iloc[-1])
