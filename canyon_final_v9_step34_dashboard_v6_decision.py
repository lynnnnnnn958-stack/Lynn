...
def tab_overview():
    ...
    risk_msg = {
        "RED": "Risk is elevated: do not convert candidates directly into trades; prioritize concentration / stress / kill-zone review.",
        "AMBER": "Moderate risk: research is fine, but keep position sizes small and continue paper trading.",
        "GREEN": "Risk is relatively mild: manual checks still required.",
    }.get(m["risk"], "")
    ...
    show_df(pd.DataFrame([
        ...
        ["2", "Option Kill Zone", "Check if it is prone to pin, IV crush, or theta bleed."],
        ["3", "Options Gamma", "Review call wall, put wall, net GEX, and gamma score."],
        ["4", "Pre-trade", "Manual check: news, earnings date, liquidity, spread."],
...
def tab_decision():
    ...
    st.markdown("This page shows the final read: it merges Gamma, Kill Zone, and Pre-trade into a single research action.")
...
def tab_gamma():
    ...
    st.markdown("This is not real dealer positioning; it is only an OI/volume-based gamma pressure proxy.")
...
def tab_kill():
    ...
    st.markdown("This section is used to avoid chasing weekly OTM options that can be killed by pin, IV crush, theta bleed, or wide spreads.")
