...
║        Institutional full version: Backtest + Live + Alpha rigor + Execution realism + Statistical depth       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  v5 Core retained:                                                               ║
...
║                                                                              ║
║  v6 New additions:                                                                   ║
║  [FIX] Bull market full position: allocate by target gross exposure, BULL_STRONG → 90%+                      ║
║  [FIX] Vol target: dynamically scale position to target annualized vol (10%)                      ║
║  [FIX] Drawdown control: >5% auto reduce position 50%, >10% reduce 75%                         ║
║  [A]   Alpha IC engine: IC/ICIR/t-stat validation, failing factors excluded                   ║
║  [E]   Execution cost realism: bid-ask spread + market impact (sqrt model) + ADV constraint           ║
...
class DataLayer:
    ...
    def _synthetic(tickers: List[str], start: str, end: str):
        ...
        Includes cross-sectional momentum structure (makes IC testing meaningful)
        """
        ...
        # Individual stocks: heterogeneous Beta + persistent momentum Alpha (makes factors valid)
        betas  = np.random.uniform(0.5, 1.9, N)
        alphas = np.random.normal(0.0002, 0.0006, N)
        # Momentum persistence: last period winners continue stronger (makes CS-Momentum IC > 0)
        ...
        print(f"[Data] Synthetic data {n} days × {N}资产（牛熊周期 + 横截面动量结构）")
        return prices, volumes, market
...
class AlphaICEngine:
    ...
    def compute_ic_series(factor_matrix: pd.DataFrame,
        ...
        """
        Compute rolling IC time series for each factor
    ...
    def icir_weights(factor_validations: Dict[str, Dict]) -> Dict[str, float]:
        """
        ICIR weighting: validated factors are combined using ICIR weights
        """
    ...
    def build_validated_alpha(prices: pd.DataFrame,
        ...
        """
        Build cross-sectional Alpha with IC validation

        Factor library:
        F1 = 12-1 month cross-sectional momentum (core factor)
        F2 = Short-term reversal (5-day)
        F3 = Volume-price directionality
        F4 = Relative strength (vs market)
        F5 = Low volatility factor

        Each factor computes IC series first; only used if it passes the threshold; weight = ICIR
        """
        ...
        # Compute IC for each factor over the past window
        window = min(252, n - 22)
        factors_raw = {}
        ...
        # Compute IC for each factor (using the IC series from the past window)
        ...
        if not weights:
            # No factors passed; fall back to equal-weight momentum
...
# ══════════════════════════════════════════════════════════════════════════════
# 3. [NEW] Execution Cost Model (resolves "Execution Realism 3/10")
# ══════════════════════════════════════════════════════════════════════════════
...
class ExecutionCostModel:
    ...
    Higher volatility and lower liquidity stocks have wider bid-ask spreads

    Market impact (Almgren-Chriss square root model):
    impact = σ_daily × √(trade_value / ADV)
    Large order costs increase nonlinearly; biggest underestimation in simple cost models

    ADV constraint:
    Single trade should not exceed max_pov = 5% of average daily volume
    ...
    def __init__(self,
                 ...
                 min_bps:   float = 2.0,    # minimum cost 2bps (ETF etc.)
    ...
    def spread_cost(self, vol_daily: float, adv_usd: float) -> float:
        """Bid-ask spread cost (as fraction of trade amount)"""
        if adv_usd <= 0:
            return self.max_bps
        # spread ∝ σ / √ADV
    ...
    def market_impact(self, vol_daily: float, adv_usd: float,
        ...
        impact = σ × √(Q/ADV)
        Q = trade amount, ADV = average daily volume in dollars
        """
    ...
    def total_cost(self, vol_daily: float, adv_usd: float,
                   trade_usd: float) -> float:
        """Total cost per trade (as fraction of trade amount)"""
    ...
    def compute_portfolio_tc(self, weights_new: pd.Series,
        ...
        """
        Calculate total transaction cost for portfolio rebalancing
        """
        total_tc  = 0.0
        tc_detail = {}
...
class StatisticalDepth:
    ...
    - Newey-West corrected Sharpe: daily returns have autocorrelation, plain Sharpe is overstated
    ...
    def newey_west_sharpe(returns: pd.Series, rf: float = 0.03,
        ...
        Daily returns have autocorrelation → plain vol is understated → plain Sharpe is overstated
        Institutional requirement: reporting NW-Sharpe is the honest standard
        """
        r = returns.dropna().values
        if len(r) < 30:
    ...
    def factor_exposure(returns: pd.Series,
        ...
        Must prove: alpha significantly > 0, and R² cannot be too high (otherwise just a high-Beta strategy)
        """
        r = returns.dropna()
        ...
        return {
            ...
            'pure_alpha':    r2 < 0.6,   # R²<60% means it's not just Beta exposure
    ...
    def print_full_report(returns: pd.Series, market: pd.Series,
        ...
        print(f"  Factor exposure: Alpha={expo['alpha_ann']:+.2%} "
              ...
              f"{'✅Pure Alpha' if expo['pure_alpha'] else '⚠️Beta-skewed'}")
        print(f"  Drawdown: MaxDD={dd_d['max_dd']:.2%} "
              f"Duration {dd_d['max_duration_days']} days "
              f"Recovery {dd_d['recovery_days']} days" if dd_d['recovery_days'] else
              f"  Drawdown: MaxDD={dd_d['max_dd']:.2%} 持续{dd_d['max_duration_days']}天 未恢复")
...
class FullStatSuite:
    ...
    def print_report(report: Dict, label: str = 'Strategy'):
        ...
        b = report.get('basic', {})
        print(f"\n  [L1] 基础指标：")
        print(f"    AnnRet:{b.get('ann_ret',0):+.2%} Vol:{b.get('ann_vol',0):.2%} "
