"""
Canyon Quant v23 — Signal & Overlay Upgrades
=============================================
Built on v21, adds:
...
def sue_scores(t_date):
    ...
    if len(recent) < 20: return neutral, np.zeros(N)
...
def compute_scores_v22(t_idx, price_arr, spy_arr, bull, breadth, t_date, eligible_set):
    ...
    # [A] Breadth-adjusted momentum weight
    # In strong breadth (>70%): pure momentum dominates
    # In weak breadth (<30%):   quality/defensive dominates
    mom_wt  = 0.25 + breadth * 0.20    # 0.25 (bear) to 0.45 (strong bull)
    qual_wt = 0.20 - breadth * 0.10    # 0.20 (bear) to 0.10 (bull)

    base = (sector_rank(mom_lt) * mom_wt +
            sector_rank(mom3m)  * (mom_wt * 0.5) +
            sector_rank(above)  * 0.08 +
            civr                * 0.08 +
            sector_rank(hi52_ratio) * 0.07 +
            eq                  * qual_wt +
            droa                * 0.05 +
            (sector_rank(idio)  * 0.05 if has_idio else np.zeros(N)))

    score = base * 0.85 + sue_arr * 0.15 + sue_bonus * 0.05
...
def run_v22(price_arr, spy_arr, date_idx, rebal_list,
    ...
    for t in rebal_list:
        ...
        bull = get_regime_v22(t_date, spy_v, spy_ma, breadth, mr, fr)
...
W = 72
print("\n" + "═"*W)
print("  v23 OOS TEST  (DD_THRESH=10%) 2018-2026")
print("═"*W)

configs = [
    ("v22 Stock-only",  dict(etf_max=0.0)),
    ("v22 + TQQQ",      dict(etf_max=0.40, has_letf=True, overlay_arr=tqqq_te)),
]
if upro_te is not None and np.isfinite(upro_te).sum() > 100:
    configs.append(("v22 + UPRO",  dict(etf_max=0.40, has_letf=True, overlay_arr=upro_te)))
...
df_main = results.get("v22 + TQQQ", results.get("v22 Stock-only"))[0]
...
print(f"{'─'*W}")
df_s = results.get("v22 Stock-only")[0]
...
print(f"  Note: drawdown stop at 10% activates automatically in all scenarios")
...
# ════════════════════════════════════════════════════════════════════════════
print("\n" + "═"*W)
print("  CANYON QUANT v23 — SCORECARD")
print("═"*W)

s_main = results["v22 + TQQQ"][1]
s_stk  = results["v22 Stock-only"][1]
rsp_r  = results["v22 + TQQQ"][2]
...
# Save
results["v22 + TQQQ"][0].to_csv(ROOT/"backtest_v23_oos.csv", index=False)
results["v22 Stock-only"][0].to_csv(ROOT/"backtest_v23_stock.csv", index=False)
print(f"  Saved: backtest_v23_oos.csv, backtest_v23_stock.csv")
print("═"*W)
