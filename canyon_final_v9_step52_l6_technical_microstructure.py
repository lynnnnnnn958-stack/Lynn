...
def universe():
    tickers = set()
    for f in ["universe_master.csv", "sector_rotation_scores.csv", "options_decision_matrix.csv", "action_cards.csv", "pre_trade_checklist.csv"]:
        df = read_csv(ROOT / f)
...
def build():
    ...
    for t in tickers:
        ...
        if len(df) < 80:
            rows.append({
                ...
                "distance_to_20d_low": np.nan,
                "technical_score": 0,
                "technical_label": "NO_TECH_EDGE",
                "reasons": "No usable price history; L6 cannot confirm timing.",
            })
            liq_rows.append({
                "ticker": t,
                "avg_20d_dollar_volume": np.nan,
                "median_20d_volume": np.nan,
                "liquidity_label": "NO_PRICE",
                "notes": "No usable price history; check manually after data refresh.",
            })
            continue
        close = df["Close"]
        vol = df["Volume"]
    ...
    if "distance_to_20d_high" not in matrix.columns:
        matrix["distance_to_20d_high"] = np.nan
    if "distance_to_20d_low" not in matrix.columns:
        matrix["distance_to_20d_low"] = np.nan
