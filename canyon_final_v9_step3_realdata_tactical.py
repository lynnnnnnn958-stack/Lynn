...
║        Institutional-grade full build: Backtest + Live + Alpha Rigor +      ║
...
║                                                                              ║
║  v6 new additions:                                                           ║
║  [FIX] Bull-market full allocation: distribute by target gross exposure,     ║
║        BULL_STRONG → 90%+                                                   ║
║  [FIX] Volatility targeting: dynamically scale positions to target           ║
║        annualized vol (10%)                                                  ║
║  [FIX] Drawdown control: auto reduce 50% above 5% DD, 75% above 10% DD     ║
║  [A]   Alpha IC engine: IC/ICIR/t-stat validation, reject factors that fail  ║
║  [E]   Execution cost realism: bid-ask spread + market impact (sqrt model)   ║
║        + ADV constraint                                                      ║
║  [S]   Statistical depth: Bootstrap Sharpe CI + Newey-West + factor exposure ║
...
class DataLayer:
    ...
    def load(tickers: List[str], start: str, end: str,
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"check network, ticker, yfinance version, or switch to Polygon/Alpha Vantage/local CSV."
            )
    ...
    def _synthetic(tickers: List[str], start: str, end: str):
        ...
        Contains cross-sectional momentum structure (makes IC tests meaningful)
        """
...
def main_v9_step3():
    ...
    sleeve_weights = sleeve_mgr.allocate_by_regime(regime)
    sleeve_mgr.print_allocation(regime)
